 ...
 * The owner sets the mint price, total supply, max mint amount and reserved supply at deployment. Owner
 * can also change the minting status, update the token URI, and withdraw contract funds from contract.
 */
...
contract CoffeNft is VRFConsumerBaseV2, ERC721URIStorage, Ownable {
    ...
    uint256 private s_reservedSupplyReqId;
    ...
    ) ERC721("CoffeNFT", "COF") VRFConsumerBaseV2(_vrfCoordinator) {
    ...
    /**
     * @dev request _mintAmount of random words from the VRF. Generates an requestId and pairs it with the
     * address of the caller
     */
    function requestNft(uint32 _mintAmount) public payable returns (uint256 requestId) {
    ...
    /**
     * @dev mints _mintAmount NFTs with random URIs to the caller of the requestNft function. Updates the
     * balance of the caller; if called by the mintReservedSupply function, updates the reserved supply.
     */
    function fulfillRandomWords(
        ...
        // updating the state variables after the fulfillment in case the VRF request fails (for example in a case of low callbackGasLimit)
    ...
    /**
     * @dev picks a random URI according to the random number generated by the VRF
     */
    function pickRandomNft(uint256 _randomNumber) public view returns (string memory) {
        ...
        } else revert CoffeNft__FailedToRandomizeNft();
        return tokenUri;
    }
     ...
     */
    // mints _mintAmount of random NFTs from reserved supply
